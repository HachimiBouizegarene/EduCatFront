<template>
  ...
  <div class="imagedefond">
    ...
    <div class="ff">
      ...
      <div class="border">
        <div class="qwe">
          <div class="upleft block" v-on:click="leftop"></div>
          <div class="upright block" v-on:click="rigtop"></div>
        </div>
        <div class="qwe">
          <div class="center mirectangle" v-on:click="center"></div>
          <div class="centerright mirectangle" v-on:click="centerright"></div>
        </div>
        <div class="qwe">
          <div class="downleft block" v-on:click="lefdown"></div>
          <div class="downright block" v-on:click="rigdown"></div>
        </div>

      </div>
      <div class="borderregle">
        Règle du jeu:<br>
        Il faut reproduire la séquence que tu as devant toi, <br>plus tu avanceras dans le jeu plus la séquence sera
        difficile.<br>
      </div>
...
export default {
  ...
  methods: {
    ...
    compare: function () {
      for (let i = 0; i < this.mysequence.length; i++) {
        if (this.mysequence[i] != this.sequence[i]) {
          ...
          this.ready = false;
          this.stop = true;
          this.$emit("finished", this.sequence.length);
          break;
        }
    ...
    centerright: function () {
      if (this.ready == true) {
        $(".centerright").fadeTo("fast", 1, function () {
          $(".centerright").fadeTo("fast", 0.5);
        });
        this.mysequence.push(5);
        this.compare();
      }
    },
    clignote: function (n) {
      ...
      switch (n) {
        ...
        case 5:
          ...
          $(".centerright").fadeTo("slow", 1, function () {
            $(".centerright").fadeTo("slow", 0.5);
          });
